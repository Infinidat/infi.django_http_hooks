...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--minutes_ago', type=int, help='check if there are more than the given amount of waiting callbacks', default=10)
    ...
    def handle(self, *args, **options):
        ...
        filter_ = dict(status=options['status'],
                       update_datetime__gt=since_when)
        ...
        if callbacks_amt >= options['callbacks_limit']:
            logger.warn('There are {} {} callbacks in the queue since {}'.format(callbacks_amt, options['status'], since_when.strftime('%Y-%m-%d %H:%M:%S')))
            sys.exit(1)
